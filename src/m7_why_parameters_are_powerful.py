...
def run_test_even_better_draw_circles():
    ...
    print('Testing  better_draw_circles:  See graphics window')
    print('--------------------------------------------------')
    even_better_draw_circles(25, 'blue', 10)
    even_better_draw_circles(10, 'purple', 5)


def even_better_draw_circles(a, b, c):
    window = rg.RoseWindow(400, 400)

    # This changes the radii variation, thickness variation and color
    # Not the most exciting but its here
    center = rg.Point(200, 200)
    for k in range(10):
        circle = rg.Circle(center, a * k)
        circle.outline_thickness = c
        circle.outline_color = b
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
